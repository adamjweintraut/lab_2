...
spotify_songs <- read.csv("datasets/spotify_songs.csv")

# "track_artist","track_popularity","playlist_genre",

spotify_songs <- spotify_songs[, c("track_popularity",
"danceability","energy","key","loudness","mode","speechiness","acousticness","instrumentalness",
"liveness","valence","tempo","duration_ms")]


spotify_songs_cor <- cor(spotify_songs)
# spotify_songs_cor

# install.packages("reshape2")
library(reshape2)
melted_spotify_songs_cor <- melt(spotify_songs_cor)
# head(melted_spotify_songs_cor)

library(ggplot2)
ggplot(data = melted_spotify_songs_cor, aes(x=Var1, y=Var2, fill=value)) +
  geom_tile()

ggplot(data=spotify_songs, aes(danceability, valence))+
    geom_point()+
    geom_smooth(method='lm', formula=y~x)

lm = lm(danceability ~ valence, spotify_songs)
summary(lm)
...
# movies <- movies[, c("score", "year","votes","budget","gross","runtime")]
# 
# NROW(movies)
# 
# movies <- movies[!is.na(movies$score), ]
# movies <- movies[!is.na(movies$year), ]
# movies <- movies[!is.na(movies$votes), ]
