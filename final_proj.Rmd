...
\
\
_Log(Gross) = Female_indicator + Log(Budget) + runtime_
\
...
\
\
_Log(Gross) = Female_indicator + Log(Budget) + runtime + Log(Title_Unqiue_Score)_
\
...
model_2 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + runtime, movies_dataset)
model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + runtime + log(title_unique_score), movies_dataset)
...
TODO: need to update the models explanations here.
