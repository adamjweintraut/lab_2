...
movies_dataset <- read.csv("datasets/movies_dataset_cpi_with_title_features.csv")


genre_budget <- movies_dataset %>% 
                group_by(genre) %>% 
                summarise(genre_mean_adj_budget = mean(adj_budget, na.rm=TRUE))


movies_dataset <- inner_join(movies_dataset, genre_budget, by = c("genre"))

#remove year 2020
# movies_dataset <- movies_dataset[movies_dataset$year != "2020", ]
# 
# movies_dataset <- movies_dataset[movies_dataset$year != "2019", ]
...
# movies_dataset <- movies_dataset %>%
#    mutate(is_longer_than_3hrs = case_when(
#       runtime_hrs >= 3 ~ 1,
#       runtime_hrs < 3 ~ 0
#   ))

movies_dataset <- movies_dataset %>% 
  mutate(year_and_inflation_adjusted_budget = adj_budget/adjusted_scaler)

names(movies_dataset)[names(movies_dataset) == 'year_and_inflation_adjusted_budget'] <- 'i_p_adjusted_budget'

names(movies_dataset)[names(movies_dataset) == 'year_and_inflation_adjusted_gross'] <- 'i_p_adjusted_gross'

#68761852.32

# movies_dataset <- movies_dataset %>%
#    mutate(writer_female_indicator = case_when(
#       writer_gender == "male" ~ 0,
#       writer_gender == "female" ~ 1
#   ))
# 
# movies_dataset <- movies_dataset %>%
#    mutate(is_same_gender = case_when(
#       writer_female_indicator == female_indicator ~ 1,
#       writer_female_indicator != female_indicator ~ 0
#   ))


# movies_dataset <- movies_dataset[movies_dataset$genre == "Comedy", ]
...
\
```{r warning=FALSE, message=FALSE, echo=FALSE}
# ggplot(movies_dataset, aes(genre_)) + 
#   geom_histogram(position="dodge", alpha=0.8) +
...
```{r warning=FALSE, message=FALSE, echo=FALSE}
# ggplot(movies_dataset, aes(year)) + 
#   geom_histogram(position="dodge", alpha=0.8) +
...
ggplot(movies_dataset, aes(max_semantic_similarity)) + 
  geom_histogram(position="dodge", alpha=0.8) +
  ggtitle("Distribution of Movie Title Unique Score")+
  scale_x_continuous(labels = comma)


ggplot(movies_dataset, aes(max_title_similarity2)) + 
  geom_histogram(position="dodge", alpha=0.8) +
  ggtitle("Distribution of Movie Title Unique Score")+
  scale_x_continuous(labels = comma)
...
For the second model we are trying to improve the fitness of the model by introducing a feature variable that has a strong correlation with our outcome varable which is gross earnings. 
\
\
The Runtime data is also highly skewed, so we apply Log transformation to it in our second model as well:
\
\
_Log(Gross Adjusted for Inflation) = Female_indicator + Log(Budget Adjusted for Inflation) + Log(Runtime)_
\
...
\
\
_Log(Gross Adjusted for Inflation) = Female_indicator + Log(Budget Adjusted for Inflation) + Runtime + Log(Title_Unqiue_Score)_
\
...
#year_and_inflation_adjusted_gross
#year_and_inflation_adjusted_budget
...
# model_2 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + writer_gender + is_same_gender, movies_dataset)

# model_2 <- lm(log(i_p_adjusted_gross) ~ female_indicator + log(i_p_adjusted_budget) + log(runtime), movies_dataset)

# model_2 <- lm(log(adjusted_gross) ~ female_indicator + log(adjusted_budget) + genre_ + director_gender + is_same_gender, movies_dataset)

# model_2 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + genre_, movies_dataset)

# model_2 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + director_gender + is_same_gender, movies_dataset)
...
# model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + log(runtime) + log(title_unique_score) +  max_semantic_similarity + max_title_similarity +  mean_semantic_similarity + mean_title_similarity + log(genre_mean_adj_budget), movies_dataset)

# model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + log(runtime) + max_title_similarity + log(genre_mean_adj_budget), movies_dataset)

model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) +  is_genre_action + is_genre_comedy + is_genre_drama + is_genre_adventure + is_genre_biography + is_genre_animation + is_genre_horror + max_title_similarity2 + is_longer_than_2hrs, movies_dataset)


# model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + writer_gender + is_same_gender + log(title_unique_score), movies_dataset)

# model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + genre_ + log(title_unique_score), movies_dataset)


# model_3 <- lm(log(i_p_adjusted_gross) ~ female_indicator + log(i_p_adjusted_budget) + log(runtime) + max_title_similarity2, movies_dataset)

# model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + log(runtime) + max_title_similarity2 + year + is_longer_than_2hrs + is_genre_drama + is_genre_comedy + is_genre_action, movies_dataset)

# model_3 <- lm(log(adj_gross) ~ female_indicator + log(adj_budget) + year, movies_dataset)


summary(model_3)

# model_4 <- lm(log(adj_gross) ~ max_semantic_similarity + max_title_similarity + len_of_title + len_of_title2 + log_len_of_title + log_len_of_title2, movies_dataset)
...
After introducing the "Runtime" variable into our second model, we don't see any improvement on R-square. P-values of other variables also seem to be affected by this new variable. Coefficient of "Log(adj budget)" does decrease slightly, meanig that its effect has been reduced by this new runtime variable.

For our third model, it seems that the "title uniqueness" feature doesn't really make much difference to the model. Coefficients of the other feature variables don't change much, so as the R-square value. The title uniqueness score has a very low p-value less that 0.01. So it seems that title uniqueness does affect the gross earnings. The coefficient of the 'Log(title_unique_score)' is positive, meaning a high score will help increase gross earnings. A high title uniqueness score means that titles are less unique actually. This translates to that a movie with a more common title tends to bring in more gross earnings.
...
movies_dataset_model <- movies_dataset[, c("gross", "budget", "female_indicator", "title_unique_score", "runtime", "year")]
...
```
In conclusion, our analysis shows strong evidence that a female lead actor brings more gross revenue to a movie than a male lead actor. In this case, it is about 13% more on average. The "Title Unique Score" also seems to have effects on the Gross, but not as much. Our model also performs Log transformation on the title unique score, the percentage change on Gross will be about +2.6%. The percentage change will be much lower if we take out the log from title unique score (~0.01%).



Limitations:
Title similarity based on the dataset, not the original population.
