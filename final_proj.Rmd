...
Here are the statistics:
``` {r warning=FALSE, message=FALSE, echo=FALSE, results='asis'}
lm = lm(log(gross) ~ female_indicator + log(budget), movies_dataset)
stargazer(
  lm,
  header=FALSE,
  type='latex',
...
```
After introducing the budget into our model, we see that both Female_indicator and Budget have a very low p-value less that 0.01. This indicates that both of these feature variables are statistically significant in determining the gross earnings of a movie. Interestingly, the coefficient of the female_indicator is now a positive value, meaning that a movie with a female lead actor actually brings in more money (about 12% more) than a movie with male lead actor. Most importantly, the R-square has hugely improved, to almost 50%.
...
lm = lm(log(gross) ~ log(title_unique_score), movies_dataset)
stargazer(
  lm,
  header=FALSE,
  type='latex',
...
```
Based on the new statistics, it seems that this feature doesn't really make much difference on our model. Coefficients of the other feature variables don't change much, so as the R-square value.
