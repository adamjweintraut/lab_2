...
In addition, we also generate a "max_title_similarity" score for how similar a movie title to the rest of the movies in the dataset. The score is a value between 0 and 1, where 1 is the highest similarity.
...
\
\
_Log(Gross Adjusted for Inflation) = Female_indicator + Log(Budget Adjusted for Inflation) + Is_Genre_Action + Is_Genre_Comedy + Is_Genre_Drama + Is_Genre_Adventure + Is_Genre_Biography + Is_Genre_Animation + Is_Genre_Horror + Max_Title_Similarity + Is_Longer_Than_2hrs_
\
